...
var hilight = "BACKGROUND-COLOR: #ffff00" //change this if you want a different highlight color when pinged (leave background there unless you want a different style)
var fontcolour = "black" //change this for different font colous
...
var greentext = 'green' //changes the text when someone quotes with ">" at the start
var punctuation = [".", ",", "\"", "'", "&", ";", ":"] //list of common punctuation, increase or decrease as you see fit
var commands = ["cmds", "etext", "greentext", "idle", "id", "eval", "updatescript", "reconnect", "pokedex", "recmsg", "setrecmsg", "sprite", "imp", "insult", "intellisult", "html", "scriptinfo"] // Array of commands, used only to make a message pop up when you use an undefined command 
...
poScript = ({
	...
	beforeSendMessage: function (msg, channel) {
		...
		if (msg.substr(0, 1) == "~" && commands.indexOf(cmd) == -1 && msg.length > 2) {
		...
		if (cmd == "insult") {
			...
			Links[26] = "Hey, "+cData.toUpperCase()+", WTF are you doing back here? I thought you got out of here! Will you shut your fucking mouth up, stop circle-jerking around in the closet?! WTF IS WRONG WITH YOU?!? Why don't you try messing with some real trolls, like Yahoo Answers people for instance instead? Oh, and grow some fucking balls and grow the fuck up too! This site is so full of trolls like you that I can't take it anymore!";
		...
		if (cmd == "pokedex") {
			...
			var ability1 = sys.ability(sys.pokeAbility(poke, 0));
        		var ability2 = sys.ability(sys.pokeAbility(poke, 1));
       			var ability3 = sys.ability(sys.pokeAbility(poke, 2));
			var border = "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~";
			if (!cData) {
